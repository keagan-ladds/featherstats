...
await db.insert(plansTable).values({ id: "PN8P5DTKBIPY", name: "Growth", stripeProductId: "prod_S2NxdHW1uxXvhX", trialPeriod: 7, usageLimits: { maxMonthlyPageviews: 50000, maxDomains: 3, maxWorkspaces: 2, dataRetentionDays: 365 } }).onConflictDoNothing();
await db.insert(plansTable).values({ id: "TbCH5CA7NofM", name: "Scale", stripeProductId: "prod_S2O2g6FWQ4p1kF", trialPeriod: 7, usageLimits: { maxMonthlyPageviews: 100000, maxDomains: 10, maxWorkspaces: 5, dataRetentionDays: 365 } }).onConflictDoNothing();

// Add initial Plan Prices
