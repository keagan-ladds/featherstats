...
// Add Initial Plans
await db.insert(plansTable).values({ id: "3B1TTA2jObKW", name: "Free", usageLimits: { maxMonthlyPageviews: 10000, maxDomains: 1, maxWorkspaces: 1, dataRetentionDays: 90 } }).onConflictDoNothing();
await db.insert(plansTable).values({ id: "PN8P5DTKBIPY", name: "Growth", usageLimits: { maxMonthlyPageviews: 50000, maxDomains: 3, maxWorkspaces: 2, dataRetentionDays: 365 } }).onConflictDoNothing();
await db.insert(plansTable).values({ id: "TbCH5CA7NofM", name: "Scale", usageLimits: { maxMonthlyPageviews: 100000, maxDomains: 10, maxWorkspaces: 5, dataRetentionDays: 365 } }).onConflictDoNothing();
