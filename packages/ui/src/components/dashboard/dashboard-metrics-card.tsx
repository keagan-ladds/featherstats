...
export default function DashboardMetricsCard({ data, className }: DashboardMetricsCardProps) {
  ...
  const total = useMemo(
    () => ({
      ...
      bounce_rate: data?.reduce((acc, curr) => acc + curr.bounce_rate, 0),
      avg_session_sec: data?.reduce((acc, curr) => acc + curr.avg_session_sec, 0),
    }),[data]
  )
        ...
        <div className="ui-grid ui-grid-cols-2 sm:ui-flex">
          {["visits", "pageviews", "bounce_rate", "avg_session_sec"].map((key) => {
            ...
            return (
              ...
              >
                <span className="ui-text-ui-xs ui-text-muted-foreground">
